...
class ExpenseItem(BaseModel):
    ...
    amount: float = Field(..., description="Monthly expense amount")
...
class DebtInput(BaseModel):
    ...
    interest_rate: float = Field(..., description="Annual interest rate")
    min_payment: Optional[float] = Field(
        None,
...
def update_financial_data(
    ...
    expenses_patch: Optional[str] = None,
    add_debts: Optional[str] = None,
    remove_debt_names: Optional[List[str]] = None,
) -> Dict[str, Any]:
    """Patch the user's financial profile in shared state."""
    ...
    if expenses_patch:
        try:
            parsed_expenses = json.loads(expenses_patch)
        except Exception:
            parsed_expenses = []

    if add_debts:
        try:
            parsed_debts = json.loads(add_debts)
        except Exception:
            parsed_debts = []
    ...
    if parsed_expenses:
        ...
        changes.append("expenses updated")
    ...
    if parsed_debts:
        ...
        fd["debts"] = existing
        changes.append(f"{len(parsed_debts)} debt(s) added")
    ...
    if remove_debt_names:
        ...
        if removed:
            changes.append(
                f"-{removed} debt(s) removed: {', '.join(remove_debt_names)}"
            )
...
1. If the user states a fact about their finances, call update_financial_data with ONLY the fields mentioned.

Examples:

User: "My income is $8000"
Call:
update_financial_data(
    monthly_income=8000
)

User: "My rent is $2000"
Call:
update_financial_data(
    expenses_patch='[
...
User: "Add a student loan of $10000 at 6%"
Call:
update_financial_data(
    add_debts='[
        {
...
User: "I paid off my car loan"
Call:
update_financial_data(
    remove_debt_names=["Car Loan"]
)

Never overwrite the entire profile.
Only update the fields explicitly mentioned.
When passing expenses_patch or add_debts to update_financial_data, ALWAYS pass them as JSON strings, not Python objects or lists.
